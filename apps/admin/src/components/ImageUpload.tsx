  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (let i = 0; i < e.target.files.length; i++) {
        ...
        const fileRef = ref(
          storage,
          `tenants/${tenantId}/products/${Date.now()}_${fileToUpload.name}`,
        );
    ...
    <div>
      <div className="flex flex-wrap gap-4 mb-4">
        {images.map((url, index) => (
          <div
            key={index}
            className="relative w-24 h-24 rounded-md overflow-hidden border border-gray-200 group"
          >
      ...
      <p className="text-xs text-gray-500">
        Recomendado: Imágenes verticales (3:4) para mejor visualización en
        móvil.
      </p>
    </div>
  );
